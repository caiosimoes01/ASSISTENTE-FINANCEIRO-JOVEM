  ...
  return (
    <div className="animate-fade-up">
      ...
      <section className="relative pt-14 pb-6 text-center overflow-hidden">
        ...
        >
          Projete seu patrimônio usando matemática financeira real, juros
          compostos e cenários inteligentes.
        </p>
      ...
      <section className="lg:col-span-5 space-y-4">
        <div className="rounded-2xl bg-zinc-900/60 border border-zinc-800/80 backdrop-blur-sm p-5 space-y-4">
          <SliderField
            label="Valor inicial" hint="quanto você já tem" prefix="R$ "
            value={initial} onChange={setInitial} min={0} max={100000} step={100}
          />
          <SliderField
            label="Aporte mensal" hint="todo mês" prefix="R$ "
            value={monthly} onChange={setMonthly} min={0} max={10000} step={50}
          />
          <SliderField
            label="Tempo" hint="horizonte" suffix=" anos"
            value={years} onChange={setYears} min={1} max={40} step={1}
          />
          <SliderField
            label="Taxa de juros anual" hint="rendimento esperado" suffix="%"
            value={rate} onChange={setRate} min={0} max={20} step={0.1}
          >
            <div className="mt-3 flex flex-wrap gap-2">
              {rateShortcuts.map((s) => {
                ...
                return (
                  ...
                  >
                    <span>{s.label}</span>
                    <span className={active ? "text-emerald-600" : "text-zinc-500"}>{s.sub}</span>
                  </button>
                );
              })}
            </div>
...
function ResultCard({
  label, value, hint, icon, iconColor,
}: { label: string; value: string; hint: string; icon: React.ReactNode; iconColor: string }) {
  return (
...
function Milestone({
  color, label, icon,
}: { color: "emerald" | "blue" | "amber"; label: string; icon?: React.ReactNode }) {
  const map = {
